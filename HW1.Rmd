...
write.csv(differ,"E:\\2021秋招\\BaiduNetdiskWorkspace\\哥大qmss\\DV\\assignment-1---winter-olympics-mmmelowang\\data\\differ.csv", row.names = FALSE)
differ2<-read.csv("E:\\2021秋招\\BaiduNetdiskWorkspace\\哥大qmss\\DV\\assignment-1---winter-olympics-mmmelowang\\data\\differ2.csv")
```
###Use Slope chart to visualize the difference:
```{r}
library(ggrepel)
...
```{r}
ggplot(data = differ2, aes(x = ranking_method, y = ranking, group = NOC)) +
  geom_line(aes(color = NOC), size = 0.1) +
  geom_point(aes(color = NOC), size = 1) +
  geom_text_repel(data = differ2 %>% filter(ranking_method == "unadj_ranking"), 
                  aes(label = paste(NOC)) , 
                  hjust = "left", 
                  fontface = "bold", 
                  size = 3, 
                  nudge_x = -.45, 
                  direction = "y") +
  geom_text_repel(data = differ2 %>% filter(ranking_method == "pop_ranking"), 
                  aes(label = paste(NOC)) , 
                  hjust = "right", 
                  fontface = "bold", 
                  size = 3, 
  ...
  labs(title = "Ranking differ by different methods")
```
